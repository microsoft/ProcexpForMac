...
//
//  Shows, for the currently-selected process, EITHER its mapped images
//  (DLL-equivalent) OR its open file descriptors (handle-equivalent),
...
//     DLLs/Handles segmented toggle that stays in sync with the main toolbar.
    ...
    private func header(model: AppModel) -> some View {
        HStack(spacing: 10) {
            ...
            Picker("", selection: Bindable(model).lowerPaneMode) {
                Text("DLLs").tag(LowerPaneMode.modules)
                Text("Handles").tag(LowerPaneMode.handles)
            }
            .pickerStyle(.segmented)
            .labelsHidden()
            .frame(width: 150)
            .help("Show mapped images (⌘D) or open handles / file descriptors (⌘H)")
    ...
    private func fetchOnce(pid: ProcessID, mode: LowerPaneMode) async {
        ...
        case .modules:
            do {
                ...
                note = rows.isEmpty
                    ? "No mapped images were returned. macOS exposes mapped regions for processes owned by the current user; others may require the privileged helper."
                    : nil
            } catch {
                guard !Task.isCancelled, model.selection == pid, model.lowerPaneMode == mode else { return }
                moduleRows = []
        ...
        case .handles:
            do {
                ...
                note = result.isEmpty
                    ? "No open file descriptors were returned, or they are not accessible for this process."
                    : nil
            } catch {
                guard !Task.isCancelled, model.selection == pid, model.lowerPaneMode == mode else { return }
                descriptors = []
...
private struct ModuleRowsTable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTableViewDataSource, NSTableViewDelegate {
        ...
        init(selection: Binding<ModuleRow.ID?>, onDoubleClick: @escaping (ModuleRow) -> Void) {
